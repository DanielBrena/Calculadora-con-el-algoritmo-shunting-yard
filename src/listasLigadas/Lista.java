...
public class Lista <T> {
	 ...
	 * @return Una lista
	 */
	public Lista<T> invierteLista(Lista<T> lista){
		Lista<T> aux = lista;
	...
	public T obtenerPrimero() throws ListaException{
		T dato = null;
		if(p != null){
			dato = p.getValor();
			p = p.getLiga();
	...
	public T obtenerPrimeroSinELiminar() throws ListaException {
		T dato = null;
		if(this.p != null){
			dato = this.p.getValor();
		}else{
			throw new ListaException("Lista vacia");
		}
		return dato;
	...
	public T obtenerUltimoSinEliminar(){
		Nodo<T> q = this.p;
		Nodo<T> t = null;
		T resultado = null;
		if(this.p.getLiga() == null){
